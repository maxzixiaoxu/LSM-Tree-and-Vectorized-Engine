...
std::unique_ptr<VecExecutor> ExecutorGenerator::GenerateVec(
  ...
  if (plan->type_ == PlanType::Project) {
    ...
    return std::make_unique<JoinVecExecutor>(db.GetOptions().exec_options,
        join_plan->predicate_.GenExpr(), join_plan->output_schema_, GenerateVec(join_plan->ch_.get(), db, txn_id), GenerateVec(join_plan->ch2_.get(), db, txn_id));
  }

   else if (plan->type_ == PlanType::HashJoin) {
    auto hash_join_plan = static_cast<const HashJoinPlanNode*>(plan);
    return std::make_unique<HashJoinVecExecutor>(db.GetOptions().exec_options,
        hash_join_plan->predicate_.GenExpr(), hash_join_plan->output_schema_, 
        hash_join_plan->ch_->output_schema_,
        hash_join_plan->ch2_->output_schema_,
        hash_join_plan->left_hash_exprs_, hash_join_plan->right_hash_exprs_,
        GenerateVec(hash_join_plan->ch_.get(), db, txn_id), 
        GenerateVec(hash_join_plan->ch2_.get(), db, txn_id));
  }
